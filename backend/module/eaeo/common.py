...
def get_brands_by_name(request, name):
    userid = request.user.id
    userclass = request.user.user_class
    if userclass == 0 or userclass == 1:
        return list(brand.objects.filter(name=name, userid=userid))
    elif userclass == 2:
        return list(brand.objects.filter(name=name, userid=userid))
    else:
        return []

def get_campaigns_by_name(request, name):
    userid = request.user.id
    userclass = request.user.user_class
    if userclass == 0 or userclass == 1:
        return list(campaign.objects.filter(name=name, userid=userid))
    elif userclass == 2:
        return list(campaign.objects.filter(name=name, userid=userid))
    else:
        return []
...
def get_all_managers(request):
    managers = list(tbl_user.objects.filter(user_class__in=[0, 1]).values())
    return managers

def get_all_managers_ids(request):
    managers = get_all_managers(request)
    ids = []
...
def get_instagram_square_images_directory_name(request, ig_id):
    directory_name = "static/d3sE4Oks3is83ngQjdENXcosd2e5pq89/"
    directory_name += "t30sv2dljwWd3esP/"
    directory_name += "ig_id_" + str(ig_id) + "/"
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)
...
def get_influencer_data_to_pdf(request, campaign_id, influencer_id, influener_ig_id):
    ...
    res['flag_url'] = '' if influencer_detail.origin_cn == '' else 'static/eaeo/43/' + influencer_detail.origin_cn.lower() + '.svg'
    tmp = ['static', 'eaeo', '43', influencer_detail.origin_cn.lower() + '.svg']
    res['flag_url_'] = '' if influencer_detail.origin_cn == '' else Path.joinpath(settings.BASE_DIR_, *tmp).as_uri()
    res['background_url'] = 'static/library/img/inf_background.jpg'
    tmp = ['static', 'library', 'img', 'inf_background.jpg']
    res['background_url_'] = Path.joinpath(settings.BASE_DIR_, *tmp).as_uri()
    try:
    ...
    new_dozenposts = []
    for item in dozenposts:
        if item['media_suare_url'] != '':
            new_dozenposts.append(item)
    tmp_dozenposts = new_dozenposts[:12]
    dozenposts = []
    for item in tmp_dozenposts:
        item['media_suare_url'] = Path(os.path.join(settings.BASE_DIR, item['media_suare_url'])).as_uri()
        dozenposts.append(item)
    ...
    res['basedir'] = request._current_scheme_host + '/'
    lgg._logging(str(request._current_scheme_host), 1, 1)
    return res
...
def store_instagram_data(request = None, ig_id = None):
    try:
        with transaction_orm.atomic():
            lgg._logging("Synchronizing step started", 1, 1)
            ...
            lgg._logging("Synchronizing step 1 passed", 1, 1)
            ...
            for item in res:
                ...
                new_params['media_suare_url'] = get_media_square_url(request, ig_id, item['id'], new_params['media_url'], new_params['media_type'])
            ...
            lgg._logging("Synchronizing step 2 passed", 1, 1)
            ...
            lgg._logging("Synchronizing step 3 passed", 1, 1)
...
def manual_sync_instagram_account(request = None, ig_id = None):
    # return store_instagram_data(request, ig_id)
    th = threading.Thread(target=store_instagram_data, args=(request, ig_id))
    th.start()

def scheduled_sync_instagram_accounts(request = None):
    influencers_ = list(influencer.objects.filter(created_from=1).values('name', 'ig_id'))
    lgg._logging("Testing a number of influencers can be sync. Cn: " + str(len(influencers_)), 1, 1)
    flag = 1
    for influencer_ in influencers_:
        ig_id = influencer_['ig_id']
        res = store_instagram_data(request, ig_id)
        if res == 0:
            flag = 0
    return flag

def manual_sync_instagram_accounts(request = None):
    th = threading.Thread(target=scheduled_sync_instagram_accounts, args=(request,))
    th.start()
...
def get_media_square_url(request, ig_id, media_id, media_url, media_type):
    if media_type == 'VIDEO':
        return ''

    directory_name = get_instagram_square_images_directory_name(request, ig_id)
    pdf_filename = directory_name + "media_id_" + str(media_id) + ".jpg"
    if os.path.exists(pdf_filename):
        return pdf_filename
    try:
        im = Image.open(requests.get(media_url, stream=True).raw)
        im = resize_img(im)
        im.save(pdf_filename)
        return pdf_filename
    except:
        return ''
...
def resize_img(img):
    ...
    if width == height:
        return img
    if width > height:
        roi = ((width - height) // 2, 0, width - ((width-height) // 2), height)
    else:
        roi = (0, (height - width) // 2, width, height - (height - width) // 2)
