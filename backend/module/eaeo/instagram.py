...
def get_user_meias(request, ig_id, influencer_detail):
    try:
        ...
        lgg._logging()
        try:
            if e.args[0]['code'] == 100 and e.args[0]['error_subcode'] == 2108006:
                api = IgProApi(app_id=settings.SOCIAL_AUTH_FACEBOOK_KEY,
                               app_secret=settings.SOCIAL_AUTH_FACEBOOK_SECRET,
                               long_term_token=mcm.get_facebook_token_for_media(request, ig_id),
                               instagram_business_id=ig_id)
                caption_comments = api.get_user_medias(
                    ig_id,
                    fields='caption, id, comments_count, like_count, media_url, media_type, permalink, timestamp, comments.fields(hidden, id, text, timestamp, media)',
                    count=50,
                    limit=50,
                    return_json=True)
                return caption_comments
