...
from module.eaeo import facebook as fcb
from module.eaeo import pdfkit as pdf
...
class IndexView(TemplateView):
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context = {}
            context['basedir'] = self.request._current_scheme_host + '/'
            context['app_id'] = settings.SOCIAL_AUTH_FACEBOOK_KEY
            self.template_name = 'main/welcome.html'
            return context

def facebook_login(request):
    authz_code = request.GET["code"]
    redirect_url = request._current_scheme_host + "/facebook_login"
    access_token = fcb.getAccessTokenFromAuthzCode(
        authz_code=authz_code,
        client_id=settings.SOCIAL_AUTH_FACEBOOK_KEY,
        client_secret=settings.SOCIAL_AUTH_FACEBOOK_SECRET,
        redirect_url=redirect_url)
    access_token = fcb.convert_shorttermaccesstoken_to_longtermaccesstoken(access_token, settings.SOCIAL_AUTH_FACEBOOK_KEY, settings.SOCIAL_AUTH_FACEBOOK_SECRET)
    # ---------- get user information with access token ---------- #
    user_info = fcb.getUserInforFromAccesstoken(access_token=access_token)
    ig_id = user_info['accounts']['data'][0]['instagram_business_account']['id']
    ...
    caption_comments = api.get_user_medias(
        ig_id,
        fields='caption, id, comments_count, like_count, media_url, media_type, permalink, timestamp, thumbnail_url, comments.fields(hidden, id, text, timestamp, media), insights.metric(reach, impressions)',
        count=50,
        limit=50,
        return_json=True)
...
@login_required()
def welcome(request):
    if request.user.user_class != 3:
        return redirect(reverse('main:index'))
    return render(request, 'main/influencer_welcome.html')
    ...
    try:
        with transaction_orm.atomic():
            ...
            if brand_info['type'] == 0:
                if len(mcm.get_brands_by_name(request, brand_info['value'])) > 0:
                    raise Exception
                new_params = {}
            ...
            if len(mcm.get_campaigns_by_name(request, cp_name)) > 0:
                raise Exception
            new_params = {}
            new_params['id'] = shortuuid.uuid()
            new_params['userid'] = userid
            ...
            if len(competitive_brand_i) > 0:
                ...
                for competitive_brand_i_item in competitive_brand_i:
                    if len(mcm.get_brands_by_name(request, competitive_brand_i_item)) > 0:
                        raise Exception
                    new_params = {}
                    new_params['id'] = shortuuid.uuid()
                    new_params['userid'] = userid
                ...
                # update current brand's competitors list with new competitors ids

                # update new campaignn's competitors list with new competitors ids
                updating_campaign = campaign.objects.get(pk=campaign_obj.id)
...
def update_campaign(request):
    try:
        with transaction_orm.atomic():
            ...
            new_params['location_id'] = json.dumps(geography)
            new_params['brand_id'] = brand_id
            new_params['competitors'] = '' if len(competitive_brand_s) == 0 else json.dumps(competitive_brand_s)
            new_params['influencers'] = '' if len(influencer_handle_s) == 0 else json.dumps(influencer_handle_s)
...
class Influencer_detailView(Campaign_detailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['analysis_able'] = True if context['influener_ig_deatil'] != -1 else False
...
class InfluencerListView(TemplateView):
    ...
    def get_influencers(self):
        ...
        for item in influencers_:
            ...
            param = {
                'id': item['id'], 'name': item['name'], 'email': item['email'], 'api_status': status,
                'created_from': item['created_from']
            }
            influencers.append(param)
        return influencers
...
def export_pdf(request):
    ...
    if 1:
        ...
        res = pdf.render_pdf(pdf_filename, data)
        if res == -1:
            return HttpResponse('failure')
...
class InfluencerView(Influencer_detailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['menu_id'] = 6
        context['influencer_id'] = influencer_id
        context['influencer_basic'] = self.get_influencer_by_id(influencer_id)

        influencer_ig_detail = self.get_influencer_ig_by_handle(context['influencer_basic']['handle'])
        if influencer_ig_detail == -1:
            context['data_collected'] = None
            return context
    ...
    def get_influencer_by_id(self, influecer_id):
        res = mcm.get_influencer_by_id(self.request, influecer_id)
        result = {}
        result['handle'] = res.name
        result['country'] = mcm.get_country_name_from_alpha2(self.request, res.origin_cn)
...
def manual_sync(request):
    try:
        ...
        mcm.manual_sync_instagram_account(request, ig_id)
        return HttpResponse('success')
    except:
        return HttpResponse('failure')

def manual_sync_all_sec(request):
    try:
        mcm.manual_sync_instagram_accounts(request)
        return HttpResponse('success')
    except:
        lgg._logging()
        return HttpResponse('failure')